...
function DashboardContent() {
  ...
  const hasInvalidInputs =
    !normalizedGoal || !Number.isFinite(parsedDays) || parsedDays < 1;

  const displayError = hasInvalidInputs
    ? "A valid learning goal and duration are required."
    : errorMessage;
  const creationElapsedSeconds = useElapsedSeconds(isCreating);
  const actionStatus =
    creationElapsedSeconds >= 10
      ? "Still creating your learning plan..."
      : creationStatus;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsCreating(true);
      setCreationStatus("Checking your learning setup...");
      ...
      async function saveToDatabase() {
        try {
          ...
          setCreationStatus("Creating your course...");
          ...
          // Generate lessons.
          setCreationStatus("Designing your day-by-day plan...");
          ...
          setCreationStatus("Saving your lessons...");
          await saveLessons(
            lessonItems.map(
  ...
  }, [normalizedGoal, parsedDays, router, hasInvalidInputs]);
